...
def leer_codigo_barras(img):
    """Recorta la zona del código y lee CODE128"""
    W, H = img.size
    crop_w = int(W * 0.40)
    crop_h = int(H * 0.15)
    crop_x = W - crop_w - 20
...
def detectar_burbujas(img, layout, num_preguntas):
    ...
    for i in range(num_preguntas):
        ...
        marcadas = sum(v > 15 for v in intensidades)  # umbral más sensible
        if marcadas == 0:
...
def leer_omr():
    ...
    # detectar número de preguntas según nombre de examen en código
    # ejemplo EXAM007ALU030 → 30 preguntas
    num_preguntas = 20
    import re
